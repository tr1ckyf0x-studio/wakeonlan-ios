...
struct HostListTableViewModel {
    ...
    mutating func insertObject(_ host: Host, at row: Int, in section: Int) {
        ...
        sections = [.mainSection(content: items, header: header(for: section), footer: footer(for: section))]
    }
    
    mutating func updateObject(_ host: Host, in row: Int, in section: Int) {
        var items = sections[section].items
        items[row] = .host(host)
        sections = [.mainSection(content: items, header: header(for: section), footer: footer(for: section))]
    }
